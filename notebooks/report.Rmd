...
# read in reviews, games, and complexity CSVs
...
The data is split into an explore set (30% split, `r nrow(explore_df)` rows) and a test dataset (70% split, `r nrow(testing_df)` rows) for our research design. We conduct exploratory data analysis on the 30% split to identify variables and relationships we want to portray in our model; then, we use the 70% split to build our models using the selection from our EDA as an effort to reduce bias.
...
- `complexity`: our main right hand side (RHS) variable that may be highly correlated with other RHS variables that are part of our overall definition of "game complexity"
- `year_published`: important to be investigated during variable selection process during modeling
- `min_players`: important to be investigated during variable selection process during modeling
- `max_players`: important to be investigated during variable selection process during modeling
- `min_playtime`: important to be investigated during variable selection process during modeling
- `max_playtime`: important to be investigated during variable selection process during modeling
- `min_age`: important to be investigated during variable selection process during modeling
- `category`: important to be investigated during variable selection process during modeling
...
We plan to use the complexity rating in our analysis, as it is intended to be a composite made up of factors defined by Board Game Geek discussed in section 2.3 of this report. Furthermore, we will consider the minimum age, minimum playtime, and maximum playtime explanatory variables as additional indicators of complexity.

Additionally, we chose to use robust standard errors clustered by users and games, allowing us to take into account this non-independence.
...
As depicted in the exploratory data analysis section, the minimum and maximum play time had large ranges and high leverage points. Thus, we chose to winsorize the max player values, limiting them to a maximum of 15. Based on skewness in the distributions, the maximum number of players, the minimum number of players, and the maximum playtime were log10 transformed. Additionally, the year published contained negative values (presumably BC) and showed high negative skewness; therefore, this variable was converted to the game age and log10 transformed: this shifted the negative skewness to positive skewness and allowed log10 tranformation to reduce it.

```{r histograms untransformed, echo=FALSE}
# par(mfrow=c(3,3))
# hist(final_df$min_players, breaks = 10, main = "Minimum Players", xlab = 'min_players')
# hist(final_df$max_players, breaks = 10, main = "Maximum Players", xlab = 'max_players')
# hist(final_df$min_playtime, breaks = 10, main = "Minimum Playtime", xlab = 'min_playtime (mins)')
# hist(final_df$max_playtime, breaks = 10, main = "Max Playtime", xlab = 'max_playtime (mins)')
# hist(final_df$min_age, breaks = 10, main = "Minimum Player Age", xlab = 'min_age (years)')
# hist(final_df$complexity, breaks = 10, main = "Complexity", xlab = 'complexity')
# hist(final_df$rating_int, breaks = 10, main = "Rating", xlab = 'rating')
# hist(final_df$year_published, main = "Year Published", xlab = 'year_published')
```

```{r histograms transformed log10, echo=FALSE}
# par(mfrow=c(3,3))
# hist(log10(final_df$min_players), breaks = 10, main = "log10(Minimum Players)", xlab = 'log10(min_players)')
# hist(log10(final_df$max_players), breaks = 10, main = "log10(Maximum Players)", xlab = 'log10(max_players)')
# hist(log10(final_df$min_playtime), breaks = 10, main = "log10(Minimum Playtime)", xlab = 'log10(min_playtime (mins))')
# hist(log10(final_df$max_playtime), breaks = 10, main = "log10(Max Playtime)", xlab = 'log10(max_playtime (mins))')
# hist(log10(final_df$min_age), breaks = 10, main = "log10(Minimum Player Age)", xlab = 'log10(min_age (years))')
# hist(log10(final_df$complexity), breaks = 10, main = "log10(Complexity)", xlab = 'log10(complexity)')
# hist(log10(final_df$rating_int), breaks = 10, main = "log10(Rating)", xlab = 'log10(rating)')
# hist(log10(final_df$game_age), breaks = 10, main = "Years Since First Published", xlab = 'log10(game age (years))')
...
pca_scree_plot <- fviz_eig(pca, addlabels = TRUE)
pca_scree_plot + labs(title = "Category Variables PCA Scree Plot")
```


## 3.1 Base Model

In the first iteration of modeling (base model), we include only the primary explanatory variable of interest, the complexity variable:
...
In the second model, we include additional explanatory variables that are part of how we define complexity:
...
In the third model, we include other control variables to account for aspects of the game that may affect user ratings:
...
2. The correlation coefficients in the correlation matrix shows that there is no perfect collinearity
...
No chain of causality is ever simple, and we have identified several omitted variables that could have an effect on our final model. They are outlined in the causal diagram below.

```{r causal graph, echo=FALSE}
